...
rulesFromSentence [] = []
rulesFromSentence (s:[]) = End s : rulesFromSentence []
rulesFromSentence (s:(sn:st)) = Step s sn : rulesFromSentence (sn:st)


rulesFromText :: [[String]] -> [GrammarRule]
--changed to deal with more than 2 sets of sentences
--rulesFromText [s, x] = concat (map rulesFromSentence [s, x])
rulesFromText (s:xs) = concat (map rulesFromSentence (s:xs))

extendByOne :: [GrammarRule] -> StrucDesc -> [StrucDesc]
extendByOne g sd = 
    case sd of
    Last s -> map (\x -> NonLast x (sd)) (predecessors g s)
    NonLast s sd -> map (\x -> NonLast x (NonLast s sd)) (predecessors g s)
--NonLast s sd for extendByOne g sd

extend :: [GrammarRule] -> Numb -> StrucDesc -> [StrucDesc]
extend g n sd = 
    case n of 
    Z -> [sd]
    S n' -> sd : concat (map (\x -> extend g n' x) (extendByOne g sd))

generate :: [GrammarRule] -> Numb -> [StrucDesc]
generate g n = 
    case n of
    Z -> []
    S n' -> concat (map (\x -> extend g n' (Last x)) (enders g))
